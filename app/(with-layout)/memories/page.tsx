...
import { Film, Play, ChevronDown, History } from "lucide-react";
...
const memoriesData = [
	{
		...
		year: "2024",
		youtubeLink:
			"https://www.youtube.com/embed/e-LnWZhW3Ck?rel=0",
	},
	{
		title: "A Glimpse Of Jeclat 2k23",
		year: "2023",
		youtubeLink:
			"https://www.youtube-nocookie.com/embed/LIVV85YfwTQ?si=AewcEauEJ4FtwQ9Q",
	},
	{
		title: "A Glimpse Of Jeclat 2k22",
		year: "2022",
		youtubeLink:
			"https://www.youtube.com/embed/cQc-4M6KWHc?si=FF69DuO1caKWnogm",
	},
	{
		title: "A Glimpse Of Jeclat 2k20",
		year: "2020",
		youtubeLink:
			"https://www.youtube.com/embed/4h9POxBsqS4?si=gLRJDJU8aZtzUH5J",
	},
	{
		title: "A Glimpse Of Jeclat 2k18",
		year: "2018",
		youtubeLink:
			"https://www.youtube.com/embed/5HtPuIoEsFU?si=jlU_szYjn-WQNF6p",
	},
];
...
export default function MemoriesPage() {
	...
	// Scroll to specific year function
	...
	return (
		<div
			className={`relative min-h-screen w-full bg-black text-[#e0e0e0] overflow-x-hidden ${cinzel.className}`}>
			{/* --- BACKGROUND (Dynamic Brightness) --- */}
			<div className="fixed bg-black inset-0 z-0 overflow-hidden pointer-events-none">
				<motion.div
					className="absolute inset-0"
					style={{ opacity: bgOpacity }} // Controls brightness via opacity on black bg
				>
					<Image
			...
			{/* --- FIXED YEAR NAVIGATION BAR (Right Side) --- */}
			<div className="fixed right-4 md:right-8 top-1/2 -translate-y-1/2 z-20 hidden lg:flex flex-col items-end gap-6">
				...
				{memoriesData.map((data) => (
                 ...
                 ${activeYear === data.year ? "bg-yellow-500 scale-125 shadow-[0_0_15px_gold]" : "bg-[#0a0502] scale-100 group-hover:border-yellow-400"}`}>
							<div
								className={`w-1.5 h-1.5 rounded-full bg-black transition-opacity ${activeYear === data.year ? "opacity-100" : "opacity-0"}`}></div>
						</div>
					</button>
				))}
			</div>
			...
			<div className="relative z-10 w-full max-w-7xl overflow-hidden mx-auto px-6 pb-14 pt-[14em]">
				...
				<motion.div
					...
					<div className="inline-flex items-center gap-2 border border-yellow-600/30 rounded-full px-4 py-1 mb-4 bg-black/40 backdrop-blur-md">
						...
						/>
						<span
							className={`${montserrat.className} text-xs uppercase tracking-widest text-yellow-500/80`}>
							The Archives
						</span>
					</div>
...
function MemoryBlock({
	...
	// Highlighting Logic & Scroll Detection
	...
	return (
		<motion.div
			...
			{/* TEXT SIDE */}
			<div
				className={`flex-1 text-center ${isEven ? "lg:text-left" : "lg:text-right"} space-y-4 px-4`}>
				{/* YEAR HIGHLIGHT SECTION */}
				<div
					className={`flex items-center gap-4 ${isEven ? "lg:justify-start" : "lg:justify-end"} justify-center transition-all duration-700`}>
					<div
						className={`h-px transition-all duration-700 ${isInView ? "w-24 bg-yellow-400 shadow-[0_0_10px_gold]" : "w-12 bg-yellow-900/40"}`}></div>

					<h2
						className={`${cormorant.className} font-bold transition-all duration-700 transform leading-none
                 ${
										isInView
               ...
               `}>
				...
				<h3
					className={`${cinzel.className} text-3xl md:text-4xl text-white font-bold drop-shadow-md transition-opacity duration-500 ${isInView ? "opacity-100" : "opacity-60"}`}>
					{data.title}
				</h3>

				<p
					className={`${montserrat.className} text-gray-400 max-w-md mx-auto lg:mx-0 leading-relaxed`}>
					Relive the magic, the music, and the mayhem. A cinematic journey back
					to the glory of {data.year}.
				</p>
			</div>
			...
			<div
				...
				<div
					className={`relative bg-[#0a0a0a] border rounded-lg overflow-hidden shadow-2xl aspect-video transition-all duration-300 z-10 ${isInView ? "border-yellow-500/60 shadow-yellow-900/20" : "border-yellow-900/40"}`}>
					{isPlaying ? (
						<iframe
							width="100%"
							height="100%"
							src={`${data.youtubeLink}${data.youtubeLink.includes("?") ? "&" : "?"}autoplay=1`}
							title={data.title}
						...
						<div
							...
							<div className="absolute inset-0 bg-black/40 group-hover/video:bg-black/20 transition-colors flex flex-col items-center justify-center gap-4">
								...
								<span
									className={`${montserrat.className} text-xs uppercase tracking-[0.3em] font-bold text-white/80 group-hover/video:text-yellow-400 transition-colors`}>
									Watch Film
								</span>
							</div>
