...
const signupSchema = z.object({
    name: z.string().min(3, "Thy name is too short for a warrior"),
    
    rollNumber: z.string()
        .length(11, "Thy identity roll number must be exactly 11 digits"),
    
    email: z.string().email("The scroll address (email) is invalid"),
    
    phone: z.string().regex(/^[0-9]{10}$/, "Signal (Phone) must be 10 digits"),
    
    department: z.string().min(1, "Choose thy Kingdom (Department)"),
    
    year: z.string().min(1, "Identify thy Era (Year)"),
    
    password: z.string()
        .min(8, "Secret Key must be at least 8 runes long")
        .regex(/[A-Z]/, "Requires at least one Uppercase rune")
        .regex(/[a-z]/, "Requires at least one Lowercase rune")
        .regex(/[0-9]/, "Requires at least one Number rune")
        .regex(/[^A-Za-z0-9]/, "Requires at least one Special symbol rune"),
});
...
const DharmaInput = React.forwardRef(({
	...
	return (
		<div className="group relative">
            ...
            {error && <p className="text-[10px] text-red-500 mt-1 italic">{error}</p>}
		</div>
	);
});

DharmaInput.displayName = "DharmaInput";

// --- REUSABLE COMPONENT: DHARMA SELECT ---
const DharmaSelect = React.forwardRef(({ icon, label, options, error, ...props }: any, ref) => (
    <div className="group relative">
        <label className={`${rajdhani.className} text-[10px] uppercase font-bold text-yellow-800 absolute -top-3 left-0`}>
            {label}
        </label>
        <div className="flex items-center border-b border-yellow-900/50 group-focus-within:border-yellow-500 pb-2 pt-1">
            <div className="mr-3 text-yellow-700">{icon}</div>
            <select ref={ref} {...props} className={`w-full bg-transparent text-yellow-100 outline-none ${montserrat.className} text-sm appearance-none cursor-pointer [&>option]:bg-[#1a0a05]`}>
...
DharmaSelect.displayName = "DharmaSelect";
