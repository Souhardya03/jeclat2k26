...
// Instead of slicing by index (which breaks if you add/remove events),
// we filter by the 'category' string defined in your data file.
const schedule = {
  prelude: eventData.filter((e) => e.type === "pre-event"),
  day1: eventData.filter((e) => e.eventInfo.category === "Day 1"),
  day2: eventData.filter((e) => e.eventInfo.category === "Day 2"),
              ...
              {parvas.map((parva) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex-1 group flex flex-col items-center gap-2 min-w-[80px] py-4 rounded-xl transition-all duration-300 ${
                      isActive ? "bg-white/10" : "hover:bg-white/5"
                    }`}
                  >
                    <span
                      className={`${mono.className} text-[10px] lg:text-[14px] tracking-widest font-bold uppercase ${
