          ...
          ${
                ...
                <div
                    ...
                    <div className="flex gap-3 items-center">
                        <Link href="/">
                            <Image src="/assets/logo.png" alt="Logo" width={40} height={40} className="drop-shadow-glow" />
                        </Link>
                        <Link href="/">
                            <Image src="/assets/jeclatfont.svg" alt="Logo" width={120} height={120} className="drop-shadow-glow" />
                        </Link>
                    </div>
                    <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="text-yellow-500 hover:text-yellow-300 transition-colors">
                    ...
                    <div className="flex flex-col p-4 gap-2">
                        {navItems.map((item) => (
                            <Link key={item.name} href={item.href} onClick={() => setMobileMenuOpen(false)}>
                                <div className={`flex items-center gap-4 p-3 rounded-lg border transition-all ${pathname === item.href ? "bg-white/10 border-yellow-500/40 text-yellow-300" : "text-yellow-100/80 border-transparent hover:bg-white/5"}`}>
                                    <span className={pathname === item.href ? "text-yellow-400" : "text-yellow-500"}>{item.icon}</span>
                                    <span className={`${murecho.className} tracking-widest text-sm`}>{item.name}</span>
                                </div>
                            </Link>
                        ))}
                    </div>
...
const NavItem = ({
    ...
    return (
        <Link href={item.href} className="group relative px-6 py-2">
            <div className="flex flex-col items-center gap-1 relative z-10">
                {/* Icon logic: brighter if active */}
                <div className={`transition-colors duration-300 transform group-hover:scale-110 drop-shadow-md ${isActive ? "text-yellow-300 scale-110" : "text-yellow-600/70 group-hover:text-yellow-300"}`}>
                    {item.icon}
                </div>

                {/* Label logic: brighter if active */}
                <span className={`${murecho.className} text-[10px] uppercase tracking-[0.2em] font-bold transition-colors ${isActive ? "text-yellow-100" : "text-yellow-700/80 group-hover:text-yellow-100"}`}>
                    {item.name}
                </span>
            </div>

            {/* Permanent Spotlight for Active, or Hover for Inactive */}
            <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-yellow-500/10 blur-xl rounded-full transition-opacity duration-500 ${isActive ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}></div>
