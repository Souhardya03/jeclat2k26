...
import { Swords, Share2, Image as ImageIcon, Shield, Sparkles, Mail, Menu, X } from 'lucide-react';
  ...
  return (
    <>
      <nav className="fixed top-4 left-0 right-0 z-50 flex justify-center px-4">
              ...
              <Link href="/">
                 ...
                 <div className="relative flex flex-col items-center">
                    ...
                    <div className="absolute -bottom-4 flex flex-col items-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 pointer-events-none">
                       <span className={`${cinzel.className} text-yellow-400 font-bold tracking-[0.2em] text-[10px]`}>MMXXV</span>
                    </div>
                 </div>
              </Link>
           </div>
        ...
        <div className={`lg:hidden flex items-center justify-between px-6 h-16 w-full max-w-md mx-auto rounded-full bg-[#0a0502]/90 backdrop-blur-xl border border-yellow-900/50 shadow-2xl transition-all duration-500 ${scrolled ? 'translate-y-0' : 'translate-y-2'}`}>
           <Link href="/">
  ...
  return (
    <Link href={item.href} className="group relative px-6 py-2">
       <div className="flex flex-col items-center gap-1 relative z-10">
          ...
          {/* Label */}
          <span className={`${murecho.className} text-[10px] uppercase tracking-[0.2em] text-yellow-700/80 group-hover:text-yellow-100 transition-colors font-bold`}>
             {item.name}
          </span>
       </div>
