    ...
    <>
      <nav className="fixed top-4 left-0 right-0 flex justify-center px-4 z-50">
        ...
        <motion.div
          ...
          className="hidden lg:flex absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[90rem] items-center justify-center gap-6 z-0 pointer-events-none text-yellow-500/60"
        >
          <motion.div
            initial={{ scaleX: 0 }}
            animate={{ scaleX: 1 }}
            transition={{ duration: 1.5, delay: 0.5 }}
            className="h-[1px] w-24 md:w-40 bg-gradient-to-r from-transparent via-yellow-500/80 to-transparent origin-right"
          ></motion.div>
          ...
          <div className="relative">
             ...
             <Crown
                size={56} 
                strokeWidth={1.2}
                className="relative drop-shadow-[0_0_15px_rgba(255,215,0,0.5)]"
             />
          </div>
          ...
          <motion.div
            ...
            animate={{ scaleX: 1 }}
            transition={{ duration: 1.5, delay: 0.5 }}
            className="h-[1px] w-24 md:w-40 bg-gradient-to-l from-transparent via-yellow-500/80 to-transparent origin-left"
          ></motion.div>
        </motion.div>

        <div
          className={`hidden z-10 lg:flex items-center justify-center w-full max-w-7xl mx-auto rounded-full border transition-all duration-500 relative overflow-visible h-20
          ${
            scrolled
              ? "bg-[#0a0502]/80 backdrop-blur-md border-yellow-600/40 shadow-[0_10px_40px_rgba(0,0,0,0.8)]"
              : "bg-[#0a0502]/60 backdrop-blur-sm border-white/10 shadow-none"
          }`}
        >
          ...
          <div className="relative z-20 -mt-2 group cursor-pointer">
            <Link href="/">
...
const NavItem = ({
  ...
  return (
    <Link href={item.href} className="group relative px-6 py-2">
      ...
      <div
        className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-yellow-500/10 blur-xl rounded-full transition-opacity duration-500 ${isActive ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
      ></div>
